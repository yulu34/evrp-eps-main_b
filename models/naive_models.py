...
class NaiveModel(nn.Module):
    ...
    def greedy_decode(self,
        ...
        # 如果存在至少一个可访问的位置，选择最近的一个
        # loc_mask = node_mask[:, :num_locs]  # [batch_size, num_locs]
        # loc_batch = loc_mask.sum(-1) > 0    # [batch_size]
        
        # 获取车辆坐标
        vehicle_coords = vehicle_feats[:, :, 1:3]  # [batch_size, num_vehicles, coord_dim]
        batch_size = vehicle_coords.size(0)
        curr_coords = vehicle_coords.gather(1, 
                                        selected_vehicle_id[:, None, None].expand(batch_size, 1, vehicle_coords.size(-1)))
        
        # 获取位置坐标并计算距离
        loc_coords = loc_feats[:, :, 1:3]  # [batch_size, num_locs, coord_dim]
        distances = torch.linalg.norm(curr_coords.expand(-1, num_locs, -1) - loc_coords, dim=-1)
        
        # 将不可访问的位置设置为非常大的距离
        distances += ~loc_mask * 1e+9
        
        # 选择最近的位置
        nearest_loc_idx = distances.min(-1)[1]  # [batch_size]
        selected_node_ids = nearest_loc_idx * loc_batch
        ...
        depot_distances = torch.linalg.norm(curr_coords - depot_coords, dim=-1)  # [batch_size, num_depots]
        nearest_depot_idx = depot_distances.min(-1)[1]  # [batch_size]
    ...
    def sample_decode(self, 
        ...
        for itr in range(num_itr):
            ...
            conflict_cost_list.append(cost_dict["conflict_cost"].view(rep_batch, batch_size))  # 替代 penalty_list.append
        ...
        #cost_list = tour_length_list # [search_width x batch_size]
        cost_list = tour_length_list + CONFLICT_COEF * conflict_cost_list  # 替代 penalty_coef * penalty_list
        ...
        min_conflict_cost = conflict_cost_list.gather(0, min_cost_idx).squeeze(0)  # 替代 min_penalty
        ...
        return {"tour_length": min_tour_length, "conflict_cost": min_conflict_cost}, vehicle_ids, node_ids, masks  # 替代 "penalty": min_penalty